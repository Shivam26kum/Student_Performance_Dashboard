...
import { 
  ...
  Lock as LockIcon, Trash2 // Added Trash icon for more clarity if needed
} from "lucide-react";
...
export default function TeacherMarks() {
  ...
  const handleDeleteExam = async (e, examToDelete) => {
    e.stopPropagation(); // Prevent selecting the exam while trying to delete it
    ...
    try {
      ...
      // If we deleted the exam we are currently looking at, reset the view
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      fetchPastExams(grade, sec, activeSubject); // Refresh list
  ...
  return (
    <div className="h-full max-h-full flex flex-col font-sans text-slate-800 p-4 overflow-hidden relative bg-white">
        ...
        <div className="fixed inset-0 z-999 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden border border-slate-200">
            ...
            <div className="p-6 grid grid-cols-2 gap-4">
              <button onClick={exportPDF} className="flex flex-col items-center gap-3 p-4 rounded-xl border border-slate-100 hover:bg-slate-50 group">
                <FileText size={20} className="text-rose-600" />
                <span className="text-[10px] font-black uppercase">PDF</span>
              </button>
              <button onClick={exportExcel} className="flex flex-col items-center gap-3 p-4 rounded-xl border border-slate-100 hover:bg-slate-50 group">
      ...
      {/* HEADER BAR */}
      <div className="shrink-0 bg-white border border-slate-200 rounded-2xl mb-4 p-5 shadow-sm">
        <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-bold tracking-tight">Examination Entry</h2>
              <p className="text-xs text-slate-400 font-medium">{activeSubject || "Select class"}</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <input 
              ...
              className={`bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm font-bold outline-none focus:border-indigo-500 ${isSubmitted ? 'text-slate-900 font-black bg-slate-100' : ''}`}
            />
            <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg px-3">
              ...
              <input 
                ...
                className="w-12 bg-transparent py-2 text-sm font-black outline-none"
              />
            </div>
            <select 
        ...
        {pastExams.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-slate-100">
            ...
            <button onClick={resetToNewEntry} className="group flex items-center gap-1 text-[10px] font-bold px-3 py-1 rounded-md border border-dashed border-indigo-200 text-indigo-600 hover:bg-indigo-50">
            ...
            {pastExams.map((exam) => (
              <div key={exam} className="group relative">
                <button
                  ...
                  className={`text-[10px] font-bold pl-3 pr-8 py-1 rounded-md border transition-all flex items-center gap-2 ${examName === exam ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-400'}`}
                >
                  {exam}
                </button>
                {/* THE CROSS SIGN (DELETE) */}
                <button 
      ...
      {/* STUDENT LIST */}
      {!selectedClass ? (
        <div className="flex-1 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center text-slate-400">
          <Layers size={48} className="mb-2 opacity-20" />
          <p className="text-sm font-bold uppercase tracking-widest">Waiting for Selection</p>
        </div>
      ) : (
        <div className="flex-1 flex flex-col min-h-0 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mb-2">
          <div className="shrink-0 px-6 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Student Records</span>
            <select value={listFilter} onChange={(e) => setListFilter(e.target.value)} className="text-[11px] font-bold bg-transparent outline-none text-indigo-600">
            ...
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-white border-b border-slate-100 z-10">
                <tr>
                  ...
                  <th className="px-4 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Student Name</th>
                ...
                {loading ? (
                  ...
                  return (
                    <tr key={s._id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-8 py-4 font-mono text-sm text-slate-400 font-bold">{s.rollNo}</td>
                        ...
                        <div className="flex items-center justify-end gap-4">
                          ...
                          <div className="relative flex items-center">
                            <input 
                              ...
                              className={`w-16 py-1 text-center font-black text-sm outline-none border-b-2 transition-all ${isSubmitted ? 'text-slate-900 border-transparent bg-transparent opacity-100' : 'border-slate-100 focus:border-indigo-500'}`}
                            />
                            <span className={`text-[10px] ml-1 font-bold ${isSubmitted ? 'text-slate-400' : 'text-slate-300'}`}>/ {totalMarks}</span>
                          </div>
                        </div>
                      </td>
                    </tr>
          ...
          {/* FOOTER */}
          <div className="shrink-0 p-5 bg-white border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              {isSubmitted ? (
                <>
                  <button onClick={() => setShowExportModal(true)} className="flex items-center gap-2 text-slate-700 px-4 py-2 text-xs font-black border border-slate-200 rounded-lg bg-slate-50 hover:bg-slate-100">
                    <Download size={14} /> EXPORT
                  </button>
                  <div className="flex items-center gap-2 text-slate-900 px-4 py-2 text-xs font-black border-2 rounded-lg bg-white border-slate-900/10">
                    <LockIcon size={14} /> PUBLISHED
                  </div>
                  <button onClick={() => setIsSubmitted(false)} className="flex items-center gap-2 text-indigo-600 px-4 py-2 text-xs font-black hover:bg-indigo-50 rounded-lg">
                    <Unlock size={14} /> EDIT
                  </button>
                </>
              ) : (
                <button onClick={handleSubmit} disabled={!examName.trim() || loading} className="bg-slate-900 text-white px-8 py-3 rounded-lg text-xs font-black hover:bg-black transition-all flex items-center justify-center gap-2 disabled:bg-slate-200 shadow-xl">
