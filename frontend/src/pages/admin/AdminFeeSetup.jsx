...
export default function AdminFeeSetup() {
  ...
  const handleInputChange = (id, field, value) => {
    // Allows empty string for better typing experience, otherwise parses float
  ...
  const handleSaveFee = async (classId, className) => {
    ...
    try {
      ...
      // CRITICAL: Ensure all fields are valid numbers before sending to backend
      const payload = {
      ...
      if(response.status === 200 || response.status === 201) {
        ...
        fetchData(); // Refresh to sync background totals
      }
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col p-8 font-sans bg-[#F8FAFC] overflow-hidden">
      ...
      <div className="shrink-0 flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium italic">Tuition, Transport, and Miscellaneous charges</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="bg-white px-5 py-2.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-lg font-black text-slate-900">₹{totalProjectedMonthly.toLocaleString()}</span>
            </div>
          </div>
          <button onClick={fetchData} className="p-3 bg-white hover:bg-slate-50 text-slate-400 rounded-xl border border-slate-200 transition-all active:scale-95 shadow-sm">
            <RefreshCcw size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 pb-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {classes.map((cls) => (
            <div key={cls._id} className="group bg-white rounded-4xl p-6 border border-slate-200 hover:border-indigo-300 hover:shadow-xl transition-all duration-300 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-5">
                ...
                <div className="flex items-center gap-1.5 text-xs text-slate-400 font-bold bg-slate-50 px-3 py-1 rounded-lg">
                  <UsersIcon size={14} /> {cls.studentCount || 0} Students
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full mt-6 bg-slate-900 hover:bg-indigo-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 shadow-lg shadow-slate-200"
              >
      ...
      {/* FOOTER */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-max">
        <div className="bg-slate-900/90 backdrop-blur-md px-6 py-2.5 rounded-full border border-slate-700 flex items-center gap-5 shadow-2xl">
          <div className="flex items-center gap-2 border-r border-slate-700 pr-5">
            <ShieldCheck size={14} className="text-emerald-400" />
            <span className="text-[10px] font-black text-white uppercase tracking-widest">Secure Ledger</span>
          </div>
